...
The user wants the whole addressable market, thousands of accounts, not a sample. Past 10,000, or when the user wants contacts at every account, hand the loop to `discolike bulk` (Flow 10) once the query is calibrated here.
...
1. **Load the list.** A saved domain list serves as inclusion scope as well as exclusion; the upload path is the same. MCP `save-exclusion-list`; CLI `discolike queries create-exclusion-list --name "crm-accounts" --domains-file crm.csv`; SDK `client.queries.create_exclusion_list(CreateExclusionListRequest(query_name="crm-accounts", domains=[...]))`. Minimum 20 domains. Names first? Flow 3.
...
"Pull every company that matches and 100,000+ emails into one sheet." Same mechanics as Flow 5 without the per-round fit review; the user has already validated the search in the app and wants volume. The CLI runs the whole loop as `discolike bulk` (CLI 0.4.0 and later), checkpointed and resumable, so every step stays an auto-approved `discolike` call: the mechanics below are what those commands do.

1. **Anchor.** `count` with the app's filters. That number is the ceiling on companies; everything below is arithmetic. Tell the user the ceiling before anything is billed. CLI: write the form as `form.json` of API parameter names (table below), then `discolike count --params-file form.json`; `--param KEY=VALUE` and named flags override the file, and the file is validated locally before the request.
2. **Companies, 10,000 per page.** 10,000 is a page size, not a total. Each page is the same `discover` call with `max_records=10000` and every earlier page excluded through `exclusion_query_id`. After each page, add its domains to one exclusion list (MCP `save-exclusion-list`, SDK `client.queries.create_exclusion_list(...)`) and rerun. Exclusion capacity is 250,000 domains per account, so one continuous run reaches about 260,000 companies; past that, split the search by state or employee band with a fresh list each. `max_records` has a floor of 20, so the last short page still asks for at least 20. CLI: `discolike bulk companies --params-file form.json --max-companies 50000 --run-name agencies --out companies.csv` does all of this, one exclusion list `agencies-round-N` per page, and rerunning with the same `--out` resumes from the CSV.
3. **Contacts, by domain slice, no ceiling.** Contacts search ignores `offset` when `results_by_company` is set, so page the contacts stage by slicing the domain list, not by offset: with `results_by_company=5`, send 2,000 domains per call (`max_records = domains x per_company`, capped at 10,000). Every domain lands in exactly one call, no overlap, no dropped companies. Persona text goes in `icp_text`, profile exclusions in `negate_summary`, and `has_email=True` keeps only rows with an address. CLI: `discolike bulk contacts --domains-file companies.csv --per-company 5 --param icp_text="..." --negate-summary "..." --out contacts.csv` slices the list at `10000 / per-company`, writes one row per contact, and records finished slices in `contacts.csv.checkpoint`; `--has-email` is on by default.
4. **Size before you pay.** `contacts count` per domain slice is free and tells the user how many people match before the paid pull. Expect the real yield to be well under the per-company cap: on a US marketing-agency list with a decision-maker persona and accountant exclusions, 5 per company returned about 2.5 emails per company (49 rows across 20 companies, 11 of them with anyone at all). Size the company pull from that ratio, not from the cap. CLI: `discolike bulk estimate --domains-file companies.csv --per-company 5` with the same persona flags as the contacts stage prints companies, matching contacts, and an upper bound at that per-company cap (the cap is applied to the total, so a company with fewer matches pulls fewer); it is free.
5. **Write as you go, resume from disk.** Append every page and every slice to the CSV as it lands, and record which slices finished. A rerun after a crash reads the CSV and skips what is there. `bulk contacts` stamps `contacts.csv.checkpoint` with the domains file, `--per-company` and filters it was written for; a rerun with any of those changed exits 2 instead of skipping slices that were never pulled, so pass `--overwrite` or a fresh `--out`. Billing is net-new only with a 90-day cache, so a resume never charges twice for the same company or contact.
