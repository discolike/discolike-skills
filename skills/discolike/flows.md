...
"Pull every company that matches and 100,000+ emails into one sheet." Same mechanics as Flow 5 without the per-round fit review; the user has already validated the search in the app and wants volume. The CLI runs the whole loop as `discolike bulk` (CLI 0.3.3 and later), checkpointed and resumable, so every step stays an auto-approved `discolike` call: the mechanics below are what those commands do.
