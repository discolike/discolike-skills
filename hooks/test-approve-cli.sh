...
expect_prompt 'discolike count | cat secrets.txt'
expect_prompt 'discolike count | head secrets'
expect_prompt 'discolike count | tail -n 5 Makefile'
expect_prompt 'discolike count | grep key secrets.txt'
expect_prompt 'discolike count | grep -i key secrets.txt'
expect_prompt 'discolike count | wc -l secrets.txt'
expect_prompt 'discolike count | sort secrets.txt'
expect_prompt 'discolike count | tr a b secrets.txt'
expect_allow 'discolike count | grep -i "domain"'
expect_allow 'discolike count | tr -d " "'
expect_allow 'discolike count | cut -d, -f1'
expect_allow 'discolike count | head -n 20'
expect_allow 'discolike count | wc -l'
expect_allow 'discolike count | jq --arg k domain -r .count'
expect_allow 'discolike count | grep -e domain'
expect_prompt 'discolike count | grep -ie key secrets.txt' 
expect_allow 'discolike count | sort -k 2 -t , | uniq -c'
expect_allow 'discolike count | grep -A 3 "domain"'
expect_allow 'discolike count | tail -n 5 | column -t -s ,'
expect_prompt 'discolike count | grep -i key secrets.txt'
expect_prompt 'discolike count | grep -e key secrets.txt'
expect_prompt 'discolike count | sort -k 2 secrets.txt'
expect_prompt 'discolike count | jq -f prog.jq'
expect_prompt 'discolike count | jq --slurpfile x y .'
expect_prompt 'discolike count | sort -o pwned'
expect_prompt 'discolike count | sort -ro pwned'
expect_prompt 'discolike count | cut -d, -f1 secrets.csv'
expect_prompt 'discolike count | head -n 5 -c 3 secrets'
expect_prompt 'discolike count | sort --files0-from=secrets.txt'
expect_prompt 'discolike count | sort --output=pwned'
expect_prompt 'discolike count | grep --file=patterns x'
expect_prompt 'discolike count | jq --rawfile=x'
expect_prompt 'discolike count | head --lines=5 secrets'
expect_allow 'discolike count | jq --raw-output .count'
expect_allow 'discolike count | grep --line-buffered domain'
expect_allow 'discolike count | sort --unique'
